...
export default function EditContent({ typeEditOrCreateForm, updateOrCreateFisherman, setUpdateOrCreateFisherman }) {
    const { id } = useParams()
    ...
    useEffect(() => {
        ...
        promiseTeamsAvailable.then((res) => {
            ...
            setDataTeamSelected({ team_id: dataTeamSelec?.team_id, name: dataTeamSelec.team.name, nameCompacted: `${dataTeamSelec.team_id} - ${dataTeamSelec.team.name}` })
    ...
    return (
        <>
            {teamSelected ? <FormVerification >
                ...
                >
                    <Typography variant="h6">Editar Apuração da Equipe Nº {teamSelected?.team_id || ''} - {dataTeamSelected?.name || ''}</Typography>
                </Box>
