...
export default function FormsTable({
    ...
    useEffect(() => {
        const promise = appInstance.get(
            `/teams/${dataApurathion?.team_id}/fishermen`
        );
        promise
            .then((res) => {
    ...
    }, [modificationInputTeam, dataApurathion]);
    ...
    async function submitCreateOrEditVerification(event) {
        ...
        const transform = valuesInputs.map((value) => {
            return {
                ...value,
                size: parseFloat(value.size).toFixed(2),
                points: parseFloat(value.points).toFixed(2),
            };
        });
        ...
        <>
            ...
            >
                <Typography variant="h6" sx={{ marginRight: 10 }}>
                    Total de pontos: {calculate.toFixed(1)}
                </Typography>
            </Box>
            <Box
                sx={{
                ...
                <Box sx={{ paddingLeft: 5 }}>
                    ...
                    >
                        <StyledButton
                            ...
                            size="large"
                            onClick={() =>
                                localStorage.removeItem("dataFormVerification")
                            }
                            disableElevation
                        >
                            Cancelar
